...
public class Path {
    ...
    public static List<Node> shortestPath(Node v, Node end) {
        ...
        // Contains an entry for each node of Settles set S and Frontier set F
        HashMap<Node, Info> SandF= new HashMap<Node, Info>();
        ...
        while (F.size() != 0) {
            ...
            var f= F.poll();
            if (F.size() == 0 && SandF.containsKey(end)) { return pathToEnd(SandF, end); }

            for (Edge e : f.exits()) {

                Node w= e.other(f);
                int distw= SandF.get(f).dist + e.length;

                if (!SandF.containsKey(w)) {
                    F.insert(w, distw);
                    SandF.put(w, new Info(distw, f));
                } else if (SandF.get(w).dist > distw) {

                    SandF.get(w).dist= distw;
                    SandF.get(w).bkptr= f;
                    F.changePriority(w, distw);
                }
            }
        }
